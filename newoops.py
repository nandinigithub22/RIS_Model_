...
class WirelessSystemSimulation:
    def __init__(self, MB, KI, KE, N, Pmax, Rmin, Jmin):
        ...
        self.position_IRs = np.array([30, 0]) + np.random.randn(self.KI, 2)
        self.position_ERs = np.array([5, 0]) + np.random.randn(self.KE, 2)
    ...
    def compute_received_signal_IR(self, vk, ue):
        ...
        for k in range(self.KI):
            ...
            y_I_k = np.dot(gH_lb, self.pk @ vk) + np.dot(gH_lr_Phi, self.H @ self.pk @ vk) + ue
    ...
    def compute_total_power_consumed(self, P_owS_B, P_owS_I):
        total_power = np.sum([np.trace(np.outer(self.pk[:, k], self.pk[:, k].conj().T)) for k in range(self.KI)])
...
# Compute SINR and rate for IR nodes
...
# Compute total power consumed and energy efficiency
P_owS_B = 20  # dBm
P_owS_I = 10  # dBm
